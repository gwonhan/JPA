...
/*필기.
*  클래스 선어부 위에 위치해ㅔ서 JPA 에서 사용되는 엔티티 클래스임을 표시한다.
*  이는 해달 클래스와 데이터 베이스의 테이블 매핑을 의미한다.
*  -기본 생성자는 필수로 작성해야 한다.
...
public class Member {
    ...
    /*필기.
    *  @Column 속성
    *  - name : 매핑할 테이블의 컬럼 이름을 지정할 수 있음
    *  - nullable: null 값 허용여부 설정 = not null 제약 조건에 해당한다. default 는(true)로 되어있음
    *  - unique : 컬럼의 유일성 제약 조건
    *            @Table 어노테이션 속성에 uniqueConstrains 속성을 추가해야한다.
    *  - columnDefinition : 직접 컬럼의 DDL 을 설정할 수 있다.
    *  -length : 문자열의 길이 = String 타입에서만 사용 (default: 255)
    * */

    @Id
    @Column(name = "member_no")
    @GeneratedValue(strategy =GenerationType.IDENTITY) /* autoIncrement 속성임*/
   ...
   private LocalDateTime enrollDate;
    ...
    /*enum 은 언제쓰냐면 딱 정해진 값들(성별, 권한, 역할) 한꺼번에 묶을 때*/
    /*필기.
    *  @Enumerated
    *  -enum 타입을 매핑하기 위해서 사용한다.
    *  -ORDINAL : ENUM 타입을 (필드의)순서대로 매핑 장점: 데이터베이스에 저장되는 데이터의 크기가 작음
