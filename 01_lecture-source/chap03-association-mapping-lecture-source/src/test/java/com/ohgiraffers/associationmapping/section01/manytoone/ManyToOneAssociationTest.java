...
public class ManyToOneAssociationTest {
    ...
    *  -> 이를 통해서 객체를 이용하여 데이터베이스의 테이블 간 관게를 매핑할 수 있다.
    *  [다중성에 의한 분류]
    ...
    *  객체간의 연관관계를 양방향으로 만들고 싶은 경우 반대 쪽에서도 필드를
    *  추가해서 참조를 보관하면 된다.
    *  하지만 엄밀하게 말해서는 양방향이 아닌 단방향 관계인 2개로 볼 수 있다.*/

    /*필기.
    *  ManyToOne 은 다수의 엔티티가 하나의 엔티티를 참조하는 상황에서 사용된다.
    *  예를들어 하나의 카테고리가 여러개의 메뉴를 가질 수 있는 상황에서
    *  메뉴 엔티티가 카테고리 엔티티를 참조하고 있는 것이다.
    *  이때, Menu 엔티티는 Many,Category 엔티티는 One 이 된다.
    *  얀관 관계를 가지는 엔티니를 조회하는 방법은
    *  1. 객체 그래프 탐색(객체 연관 관계를 사용한 조회)(참조 연산자 . 을 통해 접근할 수 있다.)
    *  2.객체 지향 쿼리(JPQL)
    *  */
    @Autowired
    private ManyToOneService manyToOneService;

    @DisplayName("N:1 연관관계 객체 크래프 탐색을 이용한 조회 테스트")
    @Test
    ...
    @DisplayName("N:1 연관관계 객체지향쿼리(JPQL) 사용 커테고리 이름 조회 테슽트")
    @Test
    void  manyToOneLPQLFindTest(){
        int menuCode =10;
    ...
    private  static Stream<Arguments> getInfo(){
        ...
        *  단, 카테고리가 존재하는 값이 아니기 떄문에
        *  부모 테이블 (tbl_category) 에 먼저 값이 들어 있어야
        *  그 카테고리 코드를 참조하는 자식 테이블(sbl_menu) 에
        *  데이터를 넣을 수 있게 된다.
        *  이 떄 필요한 것이 !!~!!!~!
        *  ManyToOne 어노테이션에 영속성 전이 설정을 해주는 것이다.
        *  영속석 진이란 특정 엔티티를 영속화 할 때
        *  연관된 엔티티도 함께 영속화를 진행한다는 것이다.
