...
public class OneToManyTests {
    ...
    void oneToManyFindTest() {
     ...
     /*필기.
        @OneToMany 일 떄 조회임에도 불구하고 @Transactional 어노테이션을 사용하는 이유
        -1 개의 엔티티가 여러갸의 엔티티를 로드해야하는 상황(Category- menuList)
        -일때는 여러개의 엔티티를 로드해야 하기 떄문에 성능상의 이슈가 발생할 수 있다.
        - 그래서 Category 엔티티를 조회할 떄  Menu 에 대한 엔티티를 바로 로드하는 것이 아닌
         일단 가지고 있다가 진짜 필요할 떄 로드하게 된다.(Lazy)
         @OneToMany(fetch=FetchType.LAZY)
         @ManyToMany  - 이 두가지는 지연로딩이 디폴트이다.
         @ManyToOne(fetch=FetchType.EAGER)
         @OneToOne      - 이 두가지는 이른로딩이 디폴트이다.
      */
